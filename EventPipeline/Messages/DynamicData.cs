...
{
    public abstract class DynamicData:DynamicObject
    {
        protected class StringEquality : IEqualityComparer<string>
        {
            public bool Ignorecase { get; set; }
            public StringEquality(bool ignorecase)
            {
                Ignorecase = ignorecase;
            }
            public bool Equals(string x, string y)
            {
                return Ignorecase ? x.Equals(y, StringComparison.OrdinalIgnoreCase) : x.Equals(y);
            }
        ...
        protected Dictionary<string,object> Dict { get; set; }
        public DynamicData(bool ignorecase)
        {
            Dict = new Dictionary<string, object>(new StringEquality(ignorecase));
        }
        public DynamicData():this(false)
        {
        }
        ...
        public static DynamicData Deserializes(string value)
        {
            DynamicData dd = null;
            if(new System.Text.RegularExpressions.Regex(@"^<[\s\S]+>$").IsMatch(value))
            {
                dd = new DynamicXml();
            }
            else
